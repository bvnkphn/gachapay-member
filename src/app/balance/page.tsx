...
// Icon mapping function for payment methods
...
const PAYMENT_METHODS = [
  {
    id: "promptpay",
    name: "PromptPay QR",
    desc: "สแกน QR Code จ่ายเงิน",
    logo: "/logos/promptpay.jpg",
    color: "#1a56db",
    disabled: false,
  },
  {
    id: "bank_transfer",
    name: "Bank Transfer",
    desc: "โอนเงินตรงเข้าบัญชีธนาคาร",
    icon: "Building2",
    color: "#0d9488",
    disabled: false,
  },
  {
    ...
    desc: "ชำระผ่านเบอร์วอลเล็ท",
    logo: "/logos/truemoney.jpg",
    color: "#f97316",
    disabled: false,
  },
];
...
const BANKS = [
  {
    ...
    accName: "บจก. กาชาเพย์ (GachaPay Co., Ltd.)",
    logo: "/logos/kbank.jpg",
    color: "#138f2d",
  },
  {
    code: "scb",
...
function PaymentFlowModal({
  ...
  const handleSubmitSlip = async () => {
    ...
    try {
      ...
      const submitResult: any = await api.submitSlip(
        txId,
        uploadResult.url,
        bankDetails?.code,
      );
      ...
      if (submitResult?.status === "verification_failed") {
        setVerificationMessage(
          submitResult?.reason || "Slip verification did not pass.",
        );
      }
  ...
  const overlay =
    "fixed inset-0 z-50 bg-black/60 dark:bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto animate-in fade-in duration-200";
  const card =
    "bg-card border border-border/80 rounded-3xl w-full max-w-md p-4 sm:p-5 relative shadow-2xl text-foreground max-h-[85vh] overflow-y-auto custom-scrollbar flex flex-col";
  ...
  const DevNav = () => (
    <div className="w-full mt-4">
      ...
      >
        {showAdminPanel
          ? "🔒 ปิดแผงควบคุมจำลอง (Close Panel)"
          : "🛠️ เปิดแผงควบคุมจำลอง (Admin Panel)"}
      </button>
            ...
            <div className="space-y-2 mb-4 text-xs bg-muted/40 border border-border/30 rounded-xl p-3">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-foreground font-semibold">
                  {bankDetails?.name ?? "ธนาคารกสิกรไทย (KBANK)"}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">ชื่อบัญชี</span>
                <span className="text-foreground font-semibold">
                  {bankDetails?.accName ?? "บจก. กาชาเพย์ (GachaPay Co., Ltd.)"}
                </span>
              </div>
              <div className="flex justify-between items-center">
                ...
                <div className="flex items-center gap-1.5">
                  <span className="text-foreground font-mono text-[11px]">
                    {bankDetails?.number ?? "123-4-56789-0"}
                  </span>
                  <button
                    onClick={() => {
                      navigator.clipboard.writeText(
                        (bankDetails?.number ?? "1234567890").replace(/-/g, ""),
                      );
              ...
              >
                ...
                <div className="space-y-1">
                  <p className="text-xs font-bold text-foreground">
                    อัปโหลดสลิปการโอนเงิน (สลิปโอนเงิน)
                  </p>
                  <p className="text-[10px] text-muted-foreground">
                    ลากและวางรูปภาพสลิปที่นี่ หรือคลิกเพื่ออัปโหลด
                  </p>
                </div>
        ...
        <div className={card}>
          ...
          <div className="flex flex-col items-center text-center py-6">
            ...
            <p className="text-xs text-muted-foreground mb-5 max-w-[280px]">
              สลิปของท่านได้รับการส่งเรียบร้อยแล้ว กรุณารอทีมงานตรวจสอบ
              <br />
            ...
            <div className="w-full space-y-2 text-xs bg-muted/40 border border-border/30 rounded-xl p-3 mb-4">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-foreground font-semibold">
                  {bankDetails?.name ?? "Bank Transfer"}
                </span>
              </div>
              <div className="flex justify-between items-center border-t border-border/20 pt-2">
                <span className="text-muted-foreground">เลขที่อ้างอิง</span>
            ...
            <div className="w-full flex items-start gap-2.5 p-3 rounded-xl bg-blue-500/5 border border-blue-500/20 mb-4">
              ...
              <div className="text-left">
                ...
                <p className="text-[10px] text-muted-foreground leading-relaxed">
                  ทีมงานจะตรวจสอบสลิปภายใน 5-15 นาที หากข้อมูลถูกต้อง ระบบจะเติม
                  Coin ให้อัตโนมัติ
                  คุณสามารถตรวจสอบสถานะได้ที่หน้าประวัติการเติมเงิน
                </p>
              </div>
            </div>
      ...
      <div className={overlay}>
        <div className={card}>
          ...
          <div className="pb-2">
            ...
            <p className="text-center text-[11px] text-muted-foreground/80 mb-3">
              {t.qrNote(method.name)}
            </p>
            <p className="text-center text-xs text-muted-foreground mb-2">
              {t.qrTimeLeft?.replace("3", String(Math.ceil(countdown / 60))) ||
                ""}
            </p>
      ...
      <div className={overlay}>
        <div className={card}>
          ...
          <div className="flex flex-col items-center text-center pt-2">
            ...
            <p className="font-bold text-green-400 text-lg mb-2">
            ...
            <div className="w-full space-y-2 text-xs mb-5 text-left bg-muted/40 border border-border/30 rounded-xl p-4">
              {[
                [t.txItem, "Top-up Balance"],
                [t.txChannel, method.name],
                [t.txTime, `${dateStr}, ${timeStr}`],
              ].map(([k, v]) => (
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <button
                    onClick={handleCopy}
                    className="px-1.5 py-0.5 rounded bg-muted hover:bg-muted/80 text-[10px] text-muted-foreground hover:text-foreground flex items-center gap-1"
                  >
                    {copied ? (
                      <Check className="w-3 h-3 text-green-400" />
                    ) : (
            ...
            <button
              onClick={onClose}
              className="w-full py-3 rounded-xl bg-green-500 hover:bg-green-600 text-white font-bold text-sm mb-3 transition-colors"
            >
              {t.backToHome}
            </button>
          </div>
    ...
    <div className={overlay}>
      <div className={card}>
        ...
        <div className="flex flex-col items-center text-center pt-2">
          ...
          <button
            onClick={onRetry}
            className="w-full py-3 rounded-xl bg-red-500 hover:bg-red-600 text-white font-bold text-sm mb-2 transition-colors flex items-center justify-center gap-2"
          >
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div className="bg-background rounded-2xl w-full max-w-sm shadow-2xl border border-border/40 overflow-hidden">
        {/* Status bar */}
        <div
          className={`h-1 w-full ${tx.status === "completed" ? "bg-emerald-500" : tx.status === "pending" ? "bg-amber-500" : "bg-rose-500"}`}
        />
        ...
        <div className="p-6">
          ...
          <div className="space-y-3 text-sm bg-muted/20 rounded-xl p-4 mb-5">
            {[
              ...
              [
                "เวลา",
                `${new Date(tx.created_at).toLocaleTimeString("th-TH", { hour: "2-digit", minute: "2-digit" })} น.`,
              ],
              ["สถานะ", statusLabel],
            ].map(([k, v]) => (
              <div key={k} className="flex justify-between items-center">
                ...
                <span
                  className={`font-semibold text-xs ${k === "สถานะ" ? statusColor : ""}`}
                >
                  {v}
                </span>
              </div>
          ...
          <button
            onClick={onClose}
            className="w-full py-2.5 rounded-xl bg-primary hover:bg-primary/90 text-white text-xs font-semibold transition-colors"
          >
            ปิด
          </button>
        </div>
      ...
      <div className="bg-background rounded-2xl w-full max-w-2xl shadow-2xl border border-border/40 overflow-hidden">
              ...
              {loading ? (
                ...
                filtered.map((tx, i) => (
                  <tr key={i}>
                    ...
                    <td className="py-3 text-right font-semibold text-xs">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-border/30">
          <p className="text-xs text-muted-foreground">
            แสดง {data.total === 0 ? 0 : (page - 1) * PAGE_SIZE + 1}–
            {Math.min(page * PAGE_SIZE, data.total)} จากทั้งหมด {data.total}{" "}
            รายการ
          </p>
          <div className="flex items-center gap-1.5">
            <button
...
export default function BalancePage() {
  ...
  const infoText = useMemo(() => {
    switch (selectedMethod) {
      ...
      case "bank_transfer": {
        const targetBank =
          BANKS.find((b) => b.code === selectedBankCode) ?? BANKS[0];
  ...
  const refreshData = useCallback(() => {
    ...
    api
      .getTopupTransactions({ limit: 100 })
  ...
  const filteredHistory = useMemo(() => {
    return txHistory.filter((tx) => {
      // 1. Status Filter
      if (statusFilter !== "all") {
        if (statusFilter === "completed" && tx.status !== "completed")
          return false;
      ...
      if (filterMethod !== "all") {
        ...
        if (filterMethod === "PromptPay" && !methodCode.includes("PromptPay"))
          return false;
        if (
          filterMethod === "Wallet" &&
          !methodCode.toLowerCase().includes("wallet")
        )
          return false;
        if (filterMethod === "Bank" && !methodCode.includes("Bank"))
          return false;
      }
  ...
  const totalPagesHistory =
    Math.ceil(filteredHistory.length / ITEMS_PER_PAGE_HISTORY) || 1;
  ...
  const fee =
    selectedMethod === "truemoney"
      ? Math.round(topupAmount * 0.015 * 100) / 100
      : 0.0;
  ...
  const handleConfirmTopup = async () => {
    ...
    setSubmitting(true);
    try {
      const tx = await api.createTopupIntent({
        amount: total,
  ...
  return (
    <div className="min-h-screen pt-16 pb-24">
      {showHistory && (
        <HistoryModal onClose={() => setShowHistory(false)} t={t} />
      )}
      {selectedTx && (
        <ReceiptModal tx={selectedTx} onClose={() => setSelectedTx(null)} />
      )}
      {showPayment && activeMethod && (
        <PaymentFlowModal
          method={{
            ...activeMethod,
            id: activeMethod.code ?? activeMethod.id,
            desc:
              activeMethod.code === "promptpay"
                ? t.promptpayDesc
                : t.truemoneyDesc,
          }}
          amount={total}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 sm:gap-6 pt-4 md:pt-0 border-t md:border-t-0 md:border-l border-border/50 pl-0 md:pl-8 flex-1 max-w-xl">
              <div className="space-y-1">
                ...
                <p className="text-sm font-bold text-emerald-600 dark:text-emerald-400">
                  {depositedBalance.toFixed(2)} COIN
                </p>
              </div>
              <div className="space-y-1">
                <p className="text-[9px] uppercase tracking-wider text-muted-foreground font-semibold">
                ...
                <p className="text-sm font-bold text-amber-600 dark:text-yellow-400">
                  {bonusBalance.toFixed(2)} COIN
                </p>
              </div>
              <div className="space-y-1 col-span-2 sm:col-span-1">
                ...
                <p className="text-sm font-bold text-foreground/90 truncate max-w-[180px]">
          ...
          <div className="mb-6 rounded-2xl border border-yellow-500/40 bg-yellow-500/10 p-4 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-yellow-400">
                  มีรายการรอชำระเงินอยู่
                </p>
                <p className="text-xs text-muted-foreground mt-0.5">
                  {pendingTx.reference_id} · ฿
                  {parseFloat(pendingTx.amount).toFixed(2)}
                </p>
              </div>
            </div>
            <div className="flex gap-2 shrink-0">
              ...
              <button
                onClick={async () => {
                  try {
                    await api.simulateTopupCancel(pendingTx.reference_id);
                  } catch {}
                  setPendingTx(null);
                  refreshData();
                }}
                ...
                {PAYMENT_METHODS.map((m) => (
                  ...
                  >
                    {m.id === "bank_transfer" ? (
                      ...
                      >
                        {getPaymentIcon(m.icon)}
                      </div>
                    ) : m.id === "promptpay" ? (
                      <div className="w-12 h-10 rounded-xl flex items-center justify-center shrink-0 overflow-hidden">
                        <img
                          src={m.logo}
                          alt={m.name}
        ...
        <div className="glass-card rounded-2xl p-6 mt-6">
          ...
          <div className="overflow-x-auto -mx-6 px-6 sm:mx-0 sm:px-0">
            <table className="w-full text-sm min-w-[700px]">
              <thead>
                <tr className="text-xs text-muted-foreground border-b border-border/30">
                  <th className="text-left pb-3 font-medium whitespace-nowrap">
                    {t.dateTime}
                  </th>
                  <th className="text-left pb-3 font-medium whitespace-nowrap">
                    เลขที่อ้างอิง
                  ...
                  <th className="text-right pb-3 font-medium whitespace-nowrap">
                    {t.topupAmountLabel}
                  </th>
                  <th className="text-right pb-3 font-medium whitespace-nowrap">
                    {t.statusLabel}
              ...
              <tbody className="divide-y divide-border/20">
                {paginatedHistory.length === 0 ? (
                  <tr>
                    <td
                      colSpan={5}
                      className="py-8 text-center text-xs text-muted-foreground whitespace-nowrap"
                    >
          ...
          {totalPagesHistory > 1 && (
            <div className="flex items-center justify-between mt-5 pt-4 border-t border-border/20 px-1">
              <p className="text-xs text-muted-foreground">
                แสดง {(historyPage - 1) * ITEMS_PER_PAGE_HISTORY + 1}–
                {Math.min(
                  historyPage * ITEMS_PER_PAGE_HISTORY,
                  filteredHistory.length,
                )}{" "}
                จากทั้งหมด {filteredHistory.length} รายการ
              </p>
              <div className="flex items-center gap-2">
                <button
                ...
                <button
                  onClick={() =>
                    setHistoryPage((p) => Math.min(totalPagesHistory, p + 1))
                  }
